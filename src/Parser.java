...
public class Parser {
    ...
    public Proposition parse(String string) {
        string = string.trim();
            ...
            while (pointer < length) {
                if (string.charAt(pointer) == '(') {
                    ...
                    if (!(stack.peek().equals("("))) {
                        ...
                        Proposition p = new BinaryProp(String.format("(%s %s %s)", p2.toString(), c.toString(), p1.toString()), (Proposition) p2, (Proposition) p1, (Connective) c);
                        stack.push(p);
                    } else {
                        stack.pop(); // clear the '('
                        Proposition p = new UnaryProp(String.format("(%s %s)", c.toString(), p1.toString()), (Proposition) p1, (Connective) c);
                        stack.push(p);
                    }
                    pointer++;
                    ...
                    switch (string.charAt(pointer + 1)) {
                        case 'a': {
                            if (string.substring(pointer + 1, pointer + 4).equals("and")) {
                                stack.push(new Connective(Connective.Type.AND));
                                pointer += 4;
                            } else return null;
                            break;
                        }
                        case 'o': {
                            if (string.substring(pointer + 1, pointer + 3).equals("or")) {
                                stack.push(new Connective(Connective.Type.OR));
                                pointer += 3;
                            } else return null;
                            break;
                        }
                        case 'n': {
                            if (string.substring(pointer + 1, pointer + 4).equals("not")) {
                                stack.push(new Connective(Connective.Type.NOT));
                                pointer += 4;
                            } else return null;
                            break;
                        }
                        case 'i': {
                            if (string.substring(pointer + 1, pointer + 6).equals("imply")) {
                                stack.push(new Connective(Connective.Type.IMPLY));
                                pointer += 6;
                            } else return null;
                            break;
                        }
                        case 'e': {
                            if (string.substring(pointer + 1, pointer + 3).equals("eq")) {
                                stack.push(new Connective(Connective.Type.EQ));
                                pointer += 3;
                            } else return null;
                            break;
                        }
                    ...
                    if (string.charAt(pointer + 1) == '_') {
                        if (string.charAt(pointer + 2) == '{') {
                            ...
                            stack.push(new PropositionalLetter(string.substring(pointer, temp + 1)));
                            pointer = temp + 1;
                        } else return null;
                    } else {
                        stack.push(new PropositionalLetter(string.charAt(pointer) + ""));
                        pointer++;
                    }
                }
            }
